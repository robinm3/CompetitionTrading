...
class Market:
    ...
    def marketBuy(self, ID, action, quantity):
        ...
        self.accounts[ID].file.write('BUY: ' + action + " x" + str(quantity) + " at " + self.currentDateTime.strftime(
            "%d/%m/%Y,%H:%M:%S") + "\n")
    ...
    def marketSell(self, ID, action, quantity):
        ...
        self.accounts[ID].addCash(cost)
        self.accounts[ID].addStock(action, -quantity)
        self.accounts[ID].file.write('SELL: ' + action + " x" + str(quantity) + " at " + self.currentDateTime.strftime(
            "%d/%m/%Y,%H:%M:%S") + "\n")
        self.accounts[ID].file.flush()
        return 0
    ...
    def limitBuy(self, ID, action, limit, quantity):
        ...
        if cost > self.accounts[ID].getCash():
            return 2
        return self.accounts[ID].addWaitList(action, True, limit, quantity,
                                             self.currentDateTime + datetime.timedelta(days=7))
    ...
    def limitSell(self, ID, action, limit, quantity):
        ...
        if quantity > self.accounts[ID].getStocks()[action]:
            return 2
        return self.accounts[ID].addWaitList(action, False, limit, quantity,
                                             self.currentDateTime + datetime.timedelta(days=7))
